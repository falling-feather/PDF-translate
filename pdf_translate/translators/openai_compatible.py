...
5. 若给出「结构保护提示」，须优先保持表格行列数、表头/行头关系、图表注与脚注关系；表格应输出为 Markdown 表格，不要线性化为普通段落；若提示包含疑似合并单元格候选，只按结构风险处理，不自行声称已确认合并或重排。
6. 若用户消息含「段尾顺延协议」，须严格按协议结构输出（含标识符与未译英文尾段）；否则只输出译文正文。
7. 不要输出解释性文字。"""
...
def _build_user_message(req: TranslationRequest) -> str:
    ...
    if req.structure_hints.strip():
        parts.append("【结构保护提示】\n" + req.structure_hints.strip())
