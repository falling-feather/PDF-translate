...
| 译后质量闭环 | 本地生成 QA 报告、局部修复计划、补丁审核、发布确认、回滚演练、正式稿替换/回滚、实验指标、双语 HTML 和结构化 PDF 译文报告，覆盖数字、引用、DocumentIR 表格形状、图注/脚注关系锚点、表格脚注绑定、术语一致性与术语冲突等基础检查 |
...
| `PDF_TRANSLATE_CORS_ORIGINS` | 对公网部署时应设置为前端域名白名单，默认 `*` 仅适合本地/内网联调 |
| `PDF_TRANSLATE_JWT_SECRET` | 生产环境建议显式设置的 JWT 签名密钥；未设置时会落到本地 `jwt_secret.txt` |
| `PDF_TRANSLATE_JWT_TTL_MINUTES` | 登录 token 有效期分钟数，默认 `720`；安全预检会检查非法配置 |
| `PDF_TRANSLATE_SECRET_KEY` | 管理端保存 API Key 时使用的本地加密密钥；建议生产环境在首次保存密钥前设置 |
| `PDF_TRANSLATE_SECRET_KEY_FILE` | 可选，从文件读取本地加密密钥；优先级低于 `PDF_TRANSLATE_SECRET_KEY` |
| `PDF_TRANSLATE_MAX_UPLOAD_MB` | Web 上传 PDF 大小上限，默认 `120`；上传会流式校验 PDF 魔数，安全预检会检查非法或默认配置 |
...
- **用户**：上传 PDF，选择每块页数、重叠、翻译后端（若管理员允许）、**串联 / 并联**、并联并发数；可**终止**进行中的任务；完成后查看**总用时**，下载 **Markdown**、**VLM复核 JSON/报告/回写结果/应用报告** 或 **zip 资料包**，并可在 Web 弹窗中逐项处理 VLM/人工复核任务，再显式应用 VLM 回写结果刷新 OCR 回写、候选 QA 和晋级 IR。
- **我的任务**：列表时间按本地时区可读格式显示；支持**收藏**（最多 3 条），收藏区与主列表风格一致；**每次进入翻译工作台**会清空并行窗口缓存、拉取列表，并对**未收藏且创建超过 24 小时**的任务执行库与磁盘清理（`queued`/`running` 不删）。  
- **管理员**：配置各后端 API Key（可与环境变量叠加/覆盖规则见设置逻辑；设置 `PDF_TRANSLATE_SECRET_KEY` 后会加密落盘）、启用后端列表、是否开放注册；审计日志为**自然语言摘要**，可展开看完整详情；可浏览全部任务并下载产物，包括后置 `VLM复核` 任务清单、复核报告、`ocr-results-v1` 回写结果和 VLM 应用报告。
...
pdf-translate experiment <论文1.pdf> [论文2.pdf ...] --output-dir <实验目录> --variants page,structure --backend echo [--sample-manifest samples.csv] [--patent-batch-only]
pdf-translate experiment-samples <论文目录或PDF...> --output samples.csv [--recursive]
```

CLI 的 `translate` / `run` 默认使用 **串联** 与当前 `pipeline` 默认参数；`--chunk-strategy structure` 为实验性结构分段路径，会基于 `DocumentIR` 结构块生成翻译块；`--ocr-results` 可传入外部或本地 OCR 执行器生成的 `ocr-results-v1` JSON；`--execute-ocr` 会在未提供结果文件时尝试执行本地 OCR 任务，并把执行摘要规范化写入 `output/ocr_results.json` 后进入 OCR 回写、候选 QA 和候选晋级门禁。表格区域的 OCR 任务会在 `ocr_tasks.json` 中携带 `table_context`、`structure_contract` 和回写子目标；若结果文件包含 `structured_cells`、`cell_bboxes`、`merged_cell_candidates` 或 `table_footnotes`，这些结构化结果会继续透传到候选 QA、晋级记录和实验指标，并接受行列、bbox 与 locked token 本地门禁。**并联、Web 专属选项**以 Web 表单为准。详见 `pdf-translate translate --help`。

`experiment` 用于专利证据链预跑：它会把多篇 PDF 与多个策略组成实验矩阵，逐次复用 `run` 管线，并汇总每次的 `experiment_metrics.json`、`run_metrics.json` 和 `cost_estimate.json`，输出 `batch_experiment_manifest.json`、`batch_experiment_summary.json`、`batch_experiment_summary.md` 与带多维人工评分列的 `batch_experiment_review.csv`。`experiment-samples` 可先按文本量、图片页、表格数字行、公式符号、多栏块分布、图注/脚注线索和实体候选生成 `samples.csv`、样本分析 JSON 和覆盖度 Markdown 报告；报告会给出申请前测试集覆盖度，提示普通、表格密集、公式密集、多栏复杂、扫描/低文本、注释/实体密集样本还缺多少。`--sample-manifest` 可读取 JSON/CSV/TSV 样本元数据，字段可包含 `source_pdf`、`sample_id`、`pdf_type`、`tags`、`notes`，也可填写 `confirmed_pdf_type`、`confirmed_tags`、`include_in_patent_batch`、`reviewer`、`review_notes`；确认字段会优先于自动建议字段进入批量实验。配合 `--patent-batch-only` 时，批量实验只运行人工标记纳入的样本，并在 JSON/Markdown 摘要中记录输入、选中、跳过数量和筛选后的 6 类样本覆盖缺口。`source_pdf` 可写相对样本清单的路径，缺省时按 PDF 入参顺序补位。人工填写评分表后，可用 `experiment-evidence --summary-json batch_experiment_summary.json` 生成 `batch_experiment_evidence.json` 与 `batch_experiment_evidence.md`，其中会汇总 OCR 结构门禁、结构关系提示、实体提示、补丁审核、修复效果对比、修复发布状态、回滚演练状态、正式替换状态和正式回滚状态。建议先用 `--backend echo --max-chunks 1` 做本地结构/指标冒烟，再切换真实翻译后端做样本对比。
...
6. **结构层**：`output/document_ir.json`、`output/structure_chunks_manifest.json`、`output/structure_qa.json`、`output/table_reconstruction.json`、`output/structure_hints_manifest.json`、`output/chunk_boundary_qa.json`、`output/chunk_strategy_comparison.json`、`output/vision_route.json`、`output/ocr_tasks.json`、`output/ocr_results.json`、`output/ocr_writeback.json`、`output/ocr_candidate_qa.json`、`output/vlm_tasks.json`、`output/vlm_review.json`、`output/vlm_results.json`、`output/vlm_apply.json`、`output/vlm_retranslation_plan.json`、`output/ocr_candidate_promotion.json`、`output/document_ir_ocr.json`、`output/document_ir_promoted.json`（结构识别、边界保护式结构分段预览、表格单元格重建证据、表格维度、续表风险、图注/表注/表格脚注/脚注归属、实体候选、实际注入翻译请求的表格/结构关系/实体提示、页面边界碎片、当前分段策略对高风险边界的保护/切开统计、固定页分段与结构分段对比、页级 OCR/VLM 路由摘要、OCR 调度任务、表格 OCR 结构契约、结构化 OCR 结果透传、OCR 结果载荷、OCR 结果回写、候选 QA、后置 VLM 复核任务、逐项复核报告、复核后的 `ocr-results-v1` 回写结果、VLM 结果应用报告、VLM 应用后的受影响分块重译计划、候选晋级和晋级 IR）
7. **译后 QA 与修复计划**：`output/qa_report.json`、`output/qa_report.md`、`output/repair_plan.json`、`output/repair_plan.md`、`output/repair_patch_review.json`、`output/repair_effectiveness.json`、`output/repair_publish.json`、`output/repair_rollback.json`、`output/repair_formal_replace.json`、`output/repair_formal_rollback.json`、`output/experiment_metrics.json`（数字/引用/DocumentIR 表格形状/图注脚注关系锚点/表格脚注绑定/术语一致性/实体保留等检查、局部修复建议、补丁审核、修复效果对比、发布确认、回滚演练、正式稿替换/回滚，以及面向专利实验的表格重建、分段边界、图像路由、OCR 任务、表格 OCR 结构契约、结构化 OCR 结果透传、回写、候选 QA 与候选晋级等跨文件质量指标）
8. **阅读输出**：`output/bilingual.html`、`output/formal_full.md`（按块展示原文、译文、QA 问题和局部修复建议；正式稿仅在显式确认后生成，并保留覆盖前备份）
...
4. **安全预检**：启动前可运行 `pdf-translate security-check`，或登录管理端访问 `/api/admin/security/preflight` 查看同源风险清单；生产环境建议在首次保存管理端 API Key 前设置 `PDF_TRANSLATE_SECRET_KEY` 或 `PDF_TRANSLATE_SECRET_KEY_FILE`。
5. **更新流程**：`git pull` → 激活 venv → `pip install -e .` → `cd frontend && npm ci && npm run build`（若仓库内未带最新 `server/static`）→ 重新启动 `python -m pdf_translate.server`。
6. **HTTPS / 域名**：可在前级使用 Caddy、IIS 或 Nginx 反代到本机端口；与应用程序配置相互独立。
...
- 大文件请注意上传体积上限（见 `PDF_TRANSLATE_MAX_UPLOAD_MB`）；Web 会先校验 PDF 文件头，再按块写入任务目录。
- 生产部署请修改默认密码、限制 CORS、设置 `PDF_TRANSLATE_JWT_SECRET` 与 `PDF_TRANSLATE_SECRET_KEY`，并考虑 HTTPS 与防火墙策略；设为生产模式后，启动时会阻断高/中风险配置，`security-check` 可提前查看风险清单。
...
| [docs/07-版本更新状态.md](./docs/07-版本更新状态.md) | 简明版本状态、完成度、已完成任务归档和下一步缺口 |
| [docs/08-VLM-fallback任务清单.md](./docs/08-VLM-fallback任务清单.md) | OCR 后置 VLM 复核任务清单、验收指标和专利证据价值说明 |
