...
6. **结构层**：`output/document_ir.json`、`output/structure_chunks_manifest.json`、`output/structure_qa.json`、`output/table_reconstruction.json`、`output/chunk_boundary_qa.json`、`output/chunk_strategy_comparison.json`、`output/vision_route.json`（结构识别、边界保护式结构分段预览、表格单元格重建证据、表格维度、续表风险、图注/表注/表格脚注/脚注归属、实体候选、页面边界碎片、当前分段策略对高风险边界的保护/切开统计、固定页分段与结构分段对比、页级 OCR/VLM 路由摘要）
7. **译后 QA 与修复计划**：`output/qa_report.json`、`output/qa_report.md`、`output/repair_plan.json`、`output/repair_plan.md`、`output/experiment_metrics.json`（数字/引用/DocumentIR 表格形状/术语一致性/实体保留等检查、局部修复建议，以及面向专利实验的表格重建、分段边界、图像路由等跨文件质量指标）
