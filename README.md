...
6. **结构层**：`output/document_ir.json`、`output/structure_chunks_manifest.json`、`output/structure_qa.json`、`output/table_reconstruction.json`、`output/structure_hints_manifest.json`、`output/chunk_boundary_qa.json`、`output/chunk_strategy_comparison.json`、`output/vision_route.json`、`output/ocr_tasks.json`、`output/ocr_results.json`、`output/ocr_writeback.json`、`output/ocr_candidate_qa.json`、`output/ocr_candidate_promotion.json`、`output/document_ir_ocr.json`、`output/document_ir_promoted.json`（结构识别、边界保护式结构分段预览、表格单元格重建证据、表格维度、续表风险、图注/表注/表格脚注/脚注归属、实体候选、实际注入翻译请求的表格/结构关系/实体提示、页面边界碎片、当前分段策略对高风险边界的保护/切开统计、固定页分段与结构分段对比、页级 OCR/VLM 路由摘要、OCR 调度任务、表格 OCR 结构契约、结构化 OCR 结果透传、OCR 结果载荷、OCR 结果回写、候选 QA、候选晋级和晋级 IR）
