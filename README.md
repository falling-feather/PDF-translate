...
| 译后质量闭环 | 本地生成 QA 报告、局部修复计划和双语 HTML，覆盖数字、引用、表格形状、术语一致性等基础检查 |
...
6. **结构层**：`output/document_ir.json`、`output/structure_chunks_manifest.json`、`output/structure_qa.json`、`output/vision_route.json`（结构识别、边界保护式结构分段预览、表格维度、页面边界碎片、页级 OCR/VLM 路由摘要）
