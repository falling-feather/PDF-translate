...
7. **译后 QA 与修复计划**：`output/qa_report.json`、`output/qa_report.md`、`output/repair_plan.json`、`output/repair_plan.md`、`output/repair_patch_review.json`、`output/repair_publish.json`、`output/repair_rollback.json`、`output/repair_formal_replace.json`、`output/repair_formal_rollback.json`、`output/experiment_metrics.json`（数字/引用/DocumentIR 表格形状/图注脚注关系锚点/表格脚注绑定/术语一致性/实体保留等检查、局部修复建议、补丁审核、发布确认、回滚演练、正式稿替换/回滚，以及面向专利实验的表格重建、分段边界、图像路由、OCR 任务、表格 OCR 结构契约、结构化 OCR 结果透传、回写、候选 QA 与候选晋级等跨文件质量指标）
