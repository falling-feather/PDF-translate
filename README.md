...
6. **结构层**：`output/document_ir.json`、`output/structure_chunks_manifest.json`、`output/structure_qa.json`、`output/vision_route.json`（结构识别、结构分段预览、表格维度、页级 OCR/VLM 路由摘要）
7. **译后 QA 与修复计划**：`output/qa_report.json`、`output/qa_report.md`、`output/repair_plan.json`、`output/repair_plan.md`（数字/引用/表格等检查，以及局部修复建议）
8. **链接**：`output/links_index.csv`
...
| `pending_review.json` | 待确认的术语或边界（若有工作流写入） |
