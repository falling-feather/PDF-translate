...
4. **安全预检**：启动前可运行 `pdf-translate security-check`，或登录管理端访问 `/api/admin/security/preflight` 查看同源风险清单。
...
- 生产部署请修改默认密码、限制 CORS、设置 `PDF_TRANSLATE_JWT_SECRET`，并考虑 HTTPS 与防火墙策略；设为生产模式后，启动时会阻断高/中风险配置，`security-check` 可提前查看风险清单。
