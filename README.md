...
| 译后质量闭环 | 本地生成 QA 报告、局部修复计划、实验指标和双语 HTML，覆盖数字、引用、表格形状、术语一致性与术语冲突等基础检查 |
...
7. **译后 QA 与修复计划**：`output/qa_report.json`、`output/qa_report.md`、`output/repair_plan.json`、`output/repair_plan.md`、`output/experiment_metrics.json`（数字/引用/表格/术语一致性/实体保留等检查、局部修复建议，以及面向专利实验的跨文件质量指标）
