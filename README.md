...
- **用户**：上传 PDF，选择每块页数、重叠、翻译后端（若管理员允许）、**串联 / 并联**、并联并发数；可**终止**进行中的任务；完成后查看**总用时**，下载 **Markdown**、**VLM复核 JSON/报告/回写结果** 或 **zip 资料包**，并可在 Web 弹窗中逐项处理 VLM/人工复核任务。
- **我的任务**：列表时间按本地时区可读格式显示；支持**收藏**（最多 3 条），收藏区与主列表风格一致；**每次进入翻译工作台**会清空并行窗口缓存、拉取列表，并对**未收藏且创建超过 24 小时**的任务执行库与磁盘清理（`queued`/`running` 不删）。  
- **管理员**：配置各后端 API Key（可与环境变量叠加/覆盖规则见设置逻辑；设置 `PDF_TRANSLATE_SECRET_KEY` 后会加密落盘）、启用后端列表、是否开放注册；审计日志为**自然语言摘要**，可展开看完整详情；可浏览全部任务并下载产物，包括后置 `VLM复核` 任务清单、复核报告和 `ocr-results-v1` 回写结果。
...
6. **结构层**：`output/document_ir.json`、`output/structure_chunks_manifest.json`、`output/structure_qa.json`、`output/table_reconstruction.json`、`output/structure_hints_manifest.json`、`output/chunk_boundary_qa.json`、`output/chunk_strategy_comparison.json`、`output/vision_route.json`、`output/ocr_tasks.json`、`output/ocr_results.json`、`output/ocr_writeback.json`、`output/ocr_candidate_qa.json`、`output/vlm_tasks.json`、`output/vlm_review.json`、`output/vlm_results.json`、`output/ocr_candidate_promotion.json`、`output/document_ir_ocr.json`、`output/document_ir_promoted.json`（结构识别、边界保护式结构分段预览、表格单元格重建证据、表格维度、续表风险、图注/表注/表格脚注/脚注归属、实体候选、实际注入翻译请求的表格/结构关系/实体提示、页面边界碎片、当前分段策略对高风险边界的保护/切开统计、固定页分段与结构分段对比、页级 OCR/VLM 路由摘要、OCR 调度任务、表格 OCR 结构契约、结构化 OCR 结果透传、OCR 结果载荷、OCR 结果回写、候选 QA、后置 VLM 复核任务、逐项复核报告、复核后的 `ocr-results-v1` 回写结果、候选晋级和晋级 IR）
