...
**维护者本地（默认不入库）**：根目录 `.gitignore` 会排除 **`plans/`** 目录及 **`DEPLOY_WINDOWS_SERVER.md`**、**`PROJECT_DESIGN.md`**。若你在本机使用这些内容，可将路线图、详细 Windows 部署步骤、架构愿景等放在 `plans/` 中自行维护；对外协作者只需阅读本 README 与 SETUP_MANUAL。
